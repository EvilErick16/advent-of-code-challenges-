# Expected output for the following input is 159
...
def createSegmentLists(line1, line2):
    ...
    for x in line1: 
        for i in range(len(x)):
            if i == 0:
                ...
                magnitude = magnitude + x[i]
            
        segment1.append(addMoves(segment1[-1], direction, int(magnitude)))
    ...
    for x in line2: 
        ...
        segment2.append(addMoves(segment2[-1], direction, int(magnitude)))

# returns coordinates of a point 
# input is current point, direction, and magnitude
def addMoves(point, dir, mag):
    x, y = point[0], point[1]
...
# Function returns true when a point is in between two other points
def isBetween(bound1, bound2, value):
    if bound1 < bound2:
...
# Function returns the intercept for 4 points that have an intercept 
def findIntercept(p1, p2, p3, p4):
    x, y = 0, 1
    if p1[x] == p2[x]:
        return p1[x], p3[y]
    if p1[y] == p2[y]:
...
# Returns the shortest Manhattan distance when given a list of points 
def findManhattanDist(intercepts):
    x,y = 0,1
    minDist = abs(0 - intercepts[0][x]) + abs(0 - intercepts[0][y])
    for point in intercepts:
        tempDist = abs(0 - point[x]) + abs(0 - point[y])
        if tempDist < minDist:
            minDist = tempDist
        
    return minDist

# returns the number of steps it takes from one point to another
def addSteps(p1, p2):
    x, y = 0, 1
    if p1[x] == p2[x]:
...
def findSteps(x_min, y_min, segment):
    x, y = 0, 1
    steps = 0
    for c in range(len(segment)):
            if c != 0:
                ...
                if isBetween(point1[x], point2[x], x_min) and y_min == point1[y] and y_min == point2[y] and point1[y] == point2[y]:
                    minPoint = []
                    minPoint.append(x_min)
                    minPoint.append(y_min)
                    steps += addSteps(point2, minPoint)
                    return steps
                
                elif isBetween(point1[y], point2[y], y_min) and x_min == point1[x] and x_min == point2[x] and point1[x] == point2[x]:
                    minPoint = []
                    minPoint.append(x_min)
                    minPoint.append(y_min)
                    steps += addSteps(point2, minPoint)
                    ...
                    steps += addSteps(point1, point2)
                    


if __name__ == '__main__':
    # for accessing a points x and y values
    x, y = 0, 1
    
    # Create segments with the given data 
    createSegmentLists(line3, line4)

    # Iterate through every segment and find intercepts
    for c in range(len(segment1)):
        if c != 0:
            point1 = segment1[c]
            point2 = segment1[c-1]
            ...
            for s in range(len(segment2)):
                if s != 0:
                    ...
                    if (point1[x] == point2[x] and point3[y] == point4[y]):
                        if (isBetween(point1[y], point2[y], point3[y]) and isBetween(point3[x], point4[x], point1[x])): 
                            intercepts.append(findIntercept(point1, point2, point3, point4))

                    elif (point1[y] == point2[y] and point3[x] == point4[x]):
                        if (isBetween(point1[x], point2[x], point3[x]) and isBetween(point3[y], point4[y], point1[1])):
                            intercepts.append(findIntercept(point1, point2, point3, point4)) 
                            
    mDist = findManhattanDist(intercepts)
    print ("Manhattan Distance for the closest interception:", mDist)

    minNumberOfSteps = 10000000
    point_x = 0
    point_y = 0
    for i in intercepts:
        steps1 = 0
        ...
        steps1 += findSteps(x_min, y_min, segment1)
        steps2 += findSteps(x_min, y_min, segment2)

        total = steps1 + steps2
        

        if (total) <  minNumberOfSteps: 
            minNumberOfSteps = total
            point_x = x_min
            point_y = y_min

    print ("Total number of steps needed to get to", point_x,",", point_y, ":", minNumberOfSteps)
